			...
			for b in buses:
				if b ['PublishedLineName'] == BUS_LINE:
					...
					if len(bus['MonitoredVehicleJourney']['OnwardCalls']['OnwardCall']) > 0:
						busStop = bus['MonitoredVehicleJourney']['OnwardCalls']['OnwardCall'][0]['StopPointName']
					...
					writer.writerow(row)busStop = b['MonitoredVehicleJourney']['OnwardCalls']['OnwardCall'][0]['StopPointName']
